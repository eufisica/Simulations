...
WHITE = (255, 255, 255)
BLACK = (44, 62, 80)
RED = (231, 76, 60)      # Potencial
BLUE = (52, 152, 219)     # Cinética
GREEN = (46, 204, 113)    # Mecânica
ORANGE = (243, 156, 18)   # Calor
GRAY = (220, 220, 220)
...
class Simulation:
    def __init__(self):
        ...
        pygame.display.set_caption("Simulador: Energia Mecânica, Cinética e Potencial - José Gonçalves (eufisica)")
        ...
        # Pista: X é fixo, apenas Y muda para evitar erros de arrasto
        ...
        # UI
        self.slider = Slider(400, 680, 200, 12, 0.0, 0.6, self.mu, "Atrito (mu)")
        self.btn_play_rect = pygame.Rect(50, 665, 100, 45)
        self.btn_reset_rect = pygame.Rect(160, 665, 140, 45)
    ...
    def update_physics(self):
        if self.is_playing and not self.dragging_ball and self.dragging_point is None:
            ...
            if self.ball_x < self.points[0][0]: self.ball_x = self.points[0][0]; self.ball_s_vel *= -0.2
            elif self.ball_x > self.points[-1][0]: self.ball_x = self.points[-1][0]; self.ball_s_vel *= -0.2
    ...
    def draw(self):
        ...
        self.screen.blit(self.font.render(play_txt, True, WHITE), (self.btn_play_rect.centerx - 20, self.btn_play_rect.centery - 7))
        self.screen.blit(self.font.render("Reiniciar", True, WHITE), (self.btn_reset_rect.centerx - 25, self.btn_reset_rect.centery - 7))
        
        self.slider.draw(self.screen, self.font)

        # CÁLCULOS E BARRAS DE ENERGIA
        # Altura relativa ao "chão" da simulação para fins de energia potencial
        h = max(0, (550 - self.ball_y) / self.px_to_meter)
        ep = 1.0 * self.g * h
        ek = 0.5 * 1.0 * ((self.ball_s_vel / self.px_to_meter)**2)
        em = ep + ek  # ENERGIA MECÂNICA
        
        # Lista para gerar as barras: (Nome, Valor, Cor)
        energies = [
            ("U (Potencial)", ep, RED), 
            ("K (Cinética)", ek, BLUE), 
            ("E. Mecânica", em, GREEN), 
            ("E. Diss.", self.thermal_energy, ORANGE)
        ]
        
        for i, (label, val, col) in enumerate(energies):
            x_pos = 720 + i*90
            pygame.draw.rect(self.screen, (200, 200, 200), (x_pos, 620, 40, 80)) # Fundo
            h_bar = min(80, val * 1.5) # Escala visual
            pygame.draw.rect(self.screen, col, (x_pos, 700 - h_bar, 40, h_bar)) # Barra
            self.screen.blit(self.font.render(label, True, BLACK), (x_pos - 10, 600))
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if my > SIM_HEIGHT: # Interação com a UI
                    ...
                    else: # Interação com a Pista/Bola
                ...
                if event.type == pygame.MOUSEMOTION:
                    ...
                    if self.dragging_point is not None:
                        # APENAS Y muda. X é fixo para evitar que os pontos "sigam" o slider.
