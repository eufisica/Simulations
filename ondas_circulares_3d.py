...
# Eixo 3D principal
ax = fig.add_axes([0.01, 0.18, 0.65, 0.80], projection="3d")
...
# Barra de cor
cbar_ax = fig.add_axes([0.67, 0.25, 0.015, 0.55])
...
# Botões
def make_button(left, bottom, label):
    ax_b = fig.add_axes([left, bottom, 0.10, 0.04], facecolor="#1e2530")
    b = Button(ax_b, label, color="#1e2530", hovercolor="#2e3540")
    b.label.set_color(label_color)
    b.label.set_fontsize(8)
    return b

btn_pause  = make_button(L,       0.38, "Pausar")
btn_reset  = make_button(L+0.12,  0.38, "Reiniciar")
btn_wire   = make_button(L,       0.32, "Grade ON")
btn_color  = make_button(L+0.12,  0.32, "Paleta")

# Radio buttons para modo
ax_radio = fig.add_axes([L, 0.05, 0.26, 0.24], facecolor="#0d1117")
radio = RadioButtons(
...
# Título
fig.text(0.72, 0.96, "Ondas Circulares 3D - José Gonçalves",
...
def cb_pause(event):
    ...
    btn_pause.label.set_text("Continuar" if state["paused"] else "Pausar")
