...
def bt_clic():
    try:
        ...
        vel_arrows = [] # lista para armazenar as setas de velocidade desenhadas
        ...
        while (ts<t):
            ...
            # desenhar componentes da velocidade em intervalos
            if int(ts / dt) % vel_interval == 0:
                ...
                # componente vx (horizontal) - azul
                arrow_vx = arrow(pos=ball.pos, axis=vec(vx*vel_scale, 0, 0), 
                                 color=color.blue, shaftwidth=0.15, headwidth=0.4, headlength=0.3)
                
                # componente vy (vertical) - vermelho
                arrow_vy = arrow(pos=ball.pos, axis=vec(0, vy*vel_scale, 0), 
                                 color=color.red, shaftwidth=0.15, headwidth=0.4, headlength=0.3)
                
                # vetor resultante da velocidade (tangente) - verde
                vel_arrow = arrow(pos=ball.pos, axis=vec(vx*vel_scale, vy*vel_scale, 0), 
                                  color=color.green, shaftwidth=0.2, headwidth=0.5, headlength=0.4)
